...
%el resultado
subplot(2,2,4)
pdf_rician_teorica = ((2*(k+1).*x)/omega).*exp(-k-(((k+1).*x.^2)/omega)).*besseli(0,2*(sqrt((k*(k+1))/omega)).*x);
plot(x,pdf_rician_teorica)
...
rayleighchan.SamplesToDisplay = '25%';%Se ve solo el 25% de las muestras para mejorar la visualizacion del grafico
numFrames = 4;
constDiag = comm.ConstellationDiagram( ...
    'Name','Señal recibida luego del desvanecimiento');
for i = 1:numFrames
